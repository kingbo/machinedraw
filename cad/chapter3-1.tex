...
观察图\ref{fig:Vxingkuai}所示的V型块，我们可以看出机件是在一个长方体上开出一个V字型的沟槽。本着化繁为简，易于理解的目的，我们暂时忽略V型块上的V字型沟槽的存在，将其简化为一个长方体。通过先完成长方体的三视图，然后在此基础上再完成V字型沟槽的三视图，二者合一即可形成最终的V型块三视图。
